...
static MT_Lock tran_lock MT_LOCK_INITIALIZER("tran_lock");
static MT_Lock tbase_lock MT_LOCK_INITIALIZER("tbase_lock");
...
int tbase_id[TRANS_TABLE_COUNT]={0};
...
//pthread_spinlock_t log_write_spinlock; 
//pthread_spinlock_t tbase_spinlock; 
...
load_tables_of_tables(sql_trans *tr, sql_schema *s)
{
	...
	for (n=s->tables.set->h; n; n = n->next) {
		...
		if (isMergeTable(t) || isReplicaTable(t) || isPartition(t)) {
...
load_table(sql_trans *tr, sql_schema *s, sqlid tid, subrids *nrs)
{
	...
	if (isRemoteWebTable(t))
		t->persistence = SQL_REMOTE_WEB;
	t->cleared = 0;
...
create_trans(sql_allocator *sa, backend_stack stk)
{
	...
	mlog( LOG_DTM_DEBUG ,JKD_TRANSACTION, 0,"create trans t:%p, t->wstime=%d",t,t->wstime  );
	return t;
}
...
#define FUNC_OIDS 2000
	...
	if (first) {
		...
		if (sql_trans_commit(tr,0) != SQL_OK) {
...
store_init(int debug, store_type store, int readonly, int singleuser, logger_settings *log_settings, backend_stack stk)
{
	...
	MT_lock_init(&tran_lock, "SQL_tran_lock");
	MT_lock_init(&tbase_lock "SQL_tbase_lock");
#endif
	MT_lock_set(&bs_lock);
...
store_manager(void)
{
	...
	fprintf(stderr, "#store_manager shared_transactions_drift is " LLFMT "\n", shared_transactions_drift);
		...
		need_flush = 0;
        	while ( store_nr_active || recover_tr ) { /* find a moment to flush ,add recover_tr for deal retry*/
            		MT_lock_unset(&bs_lock);
			if (GDKexiting())
				return;
            		MT_sleep_ms(timeout);
		...
		//strncpy( CurrLoggerGid, GlobalLoggerGid,TMGIDSIZE);
		res = logger_funcs.restart();
...
schema_dup(sql_trans *tr, int flag, sql_schema *os, sql_trans *o)
{
	...
	if (os->tables.set) {
		...
		for (n = os->tables.set->h; n; n = n->next) {
			...
			if (ot->persistence != SQL_LOCAL_TEMP && (isMergeTable(ot) || isReplicaTable(ot) || isPartition(ot)))
...
static int
validate_tables(sql_schema *s, sql_schema *os, sql_trans *tr )
{
	node *n, *o, *p;
	int ok = LOG_OK;
	int i = 0;
	...
	if (cs_size(&s->tables))
		for (n = s->tables.set->h; n; n = n->next) {
			...
			if (ot && isKindOfTable(ot) && isKindOfTable(t)) {
				...
				MT_lock_set(&tbase_lock);
				for( i = 0; i<MAX_CLIENT; i++ )//TODO: for loop can be changed  more efficient 
				{
					if( tbase_id[i] == t->base.id ) 
					{
						mlog(LOG_DTM_DEBUG, JKD_TRANSACTION,0 , "%s concurrency conflicts,id=%d",t->base.name,t->base.id );
						MT_lock_unset(&tbase_lock);
						return 0;
					}
				}
				for( i = 0; i<MAX_CLIENT; i++)
				{
					if( tbase_id[i] == 0 ) 
					{						 	
						 tbase_id[i] = t->base.id;
						 mlog(LOG_DTM_DEBUG, JKD_TRANSACTION,0 , "tbase_id[%d] = %d" ,i ,tbase_id[i]  );
						 int j=0;
						 while( tr->tbase_id_indx[j] != -1 && j<TRANS_TABLE_COUNT )
						 {
						    j++;
						 }
						  if(j >= TRANS_TABLE_COUNT )
						  {
						  	mlog(LOG_ERROR_M, JKD_TRANSACTION,0 , "error failed,TRANS_TABLE_COUNT is too small!" );
						  	return 0;
						  }
						  tr->tbase_id_indx[j]=i;
				...
				for (o = t->columns.set->h, p = ot->columns.set->h; o && p; o = o->next, p = p->next) {
					...
					mlog(LOG_DTM_DEBUG, JKD_TRANSACTION,0 , "%s c  %s,%d: wtime %d , rtime %d",t->base.name,c->base.name,c->base.id, c->base.wtime ,c->base.rtime );
					mlog(LOG_DTM_DEBUG, JKD_TRANSACTION,0 , "%s oc %s,%d: wtime %d , rtime %d",t->base.name,oc->base.name,c->base.id, oc->base.wtime , oc->base.wtime );
...
sql_trans_create(backend_stack stk, sql_trans *parent, const char *name)
{
	...
	}mlog( LOG_DTM_DEBUG ,JKD_TRANSACTION, 0,"tr:%p  tr->parent:%p",tr ,tr->parent );
	tr->trans_state = TRANS_DEFAULT;
	return tr;
}
...
int
sql_trans_commit(sql_trans *tr ,int redo_log_off )
{
	int ok = LOG_OK;
	...
	MT_lock_unset(&tran_lock);

	// set trans state
	if(ok == LOG_OK) {
		tr->trans_state = TRANS_COMMITED;
	} else {
		tr->trans_state = TRANS_ABORT;	
	}
...
{
	str seq_pos = NULL;
	/* GP's implicit sequence should be userschema.xxxx_seq, 
 	 * we need to change the code for both gp and monetdb */
	...
	if (col->def && (seq_pos = strstr(col->def, next_value_for))) {
		...
		char *seq_full_name = seq_pos + (strlen(next_value_for)); /* "schema"."seq_name" */
		char *seq_name_dot_pos = strstr(seq_full_name, ".");
		char *seq_name = _STRDUP(seq_name_dot_pos + strlen(".\""));
		sql_schema *s = col->t->s;
		node *n = NULL;
		seq_name[strlen(seq_name)-1] = '\0';
		n = cs_find_name(&s->seqs, seq_name);
		seq = find_sql_sequence(s, seq_name);
		
		if (seq && sql_trans_get_dependency_type(tr, seq->base.id, BEDROPPED_DEPENDENCY) != -1) {
			sys_drop_sequence(tr, seq, drop_action);		
			seq->base.wtime = s->base.wtime = tr->wtime = tr->wstime;
			cs_del(&s->seqs, n, seq->base.flag);
		}

		if (seq && sql_trans_get_dependency_type(tr, col->base.id, INDEX_DEPENDENCY) != -1) {
			sys_drop_sequence(tr, seq, drop_action);
			seq->base.wtime = s->base.wtime = tr->wtime = tr->wstime;
			cs_del(&s->seqs, n, seq->base.flag);
		}

		if (seq && sql_trans_get_dependency_type(tr, col->base.id, KEY_DEPENDENCY) != -1) {
			sys_drop_sequence(tr, seq, drop_action);
			seq->base.wtime = s->base.wtime = tr->wtime = tr->wstime;
			cs_del(&s->seqs, n, seq->base.flag);
		}
...
sys_table_del_tables(sql_trans *tr, sql_table *t, int drop_action)
{
	...
	if (cs_size(&t->tables)) {
		for (n = t->tables.set->h; n; ) {
			...
			if (t->type == tt_merge_table || t->type == tt_partition) {
				sql_trans_drop_table(tr, pt->s, pt->base.id, drop_action ? DROP_CASCADE_START : DROP_RESTRICT);
			}
		}
	}
}
...
sql_schema *
sql_trans_create_schema_with_id(sql_trans *tr, int id,const char *name, int auth_id, int owner)
{
	sql_schema *s = SA_ZNEW(tr->sa, sql_schema);
	sql_table *sysschema = find_sql_table(find_sql_schema(tr, "sys"), "schemas");
...
/* create_sql_idx_with_id add for retry create index */
sql_idx *
create_sql_idx_with_id(sql_allocator *sa, sql_table *t,int id, const char *name, idx_type it)
{
	sql_idx *ni = SA_ZNEW(sa, sql_idx);

	base_init(sa, &ni->base, id, TR_NEW, name);
...
/* create_sql_column_with_id add for retry create table */
sql_column *
create_sql_column_with_id(sql_allocator *sa,int id, sql_table *t, const char *name, sql_subtype *tpe)
{
	sql_column *col = SA_ZNEW(sa, sql_column);
...
void
sql_trans_drop_table_retry(sql_trans *tr, sql_schema *s, int id, int drop_action)
{
	node *n = find_sql_table_node(s, id);
	sql_table *t = n->data;
...
sql_table *
sql_trans_alter_table_type(sql_trans *tr, sql_table *t, int type)
{
	if (t->type != type) {
		sql_schema *syss = find_sql_schema(tr, isGlobal(t)?"sys":"tmp"); 
		sql_table *systable = find_sql_table(syss, "_tables");
		...
		if (rid == oid_nil)
			return NULL;
		table_funcs.column_update_value(tr, find_sql_column(systable, "type"), rid, &type);
		t->type = type;
		t->persistence = SQL_MERGE_TABLE;
		t->base.wtime = t->s->base.wtime = tr->wtime = tr->wstime;
		if (isGlobal(t)) 
			tr->schema_updates ++;
	}
...
sql_idx *
sql_trans_create_idx_with_id(sql_trans *tr, sql_table *t, const char *name,int id ,idx_type it)
{
	/* can only have idxs between persistent tables */
	sql_idx *ni = SA_ZNEW(tr->sa, sql_idx);
	sql_schema *syss = find_sql_schema(tr, isGlobal(t)?"sys":"tmp");
	...
	assert(name);
	base_init(tr->sa, &ni->base, id, TR_NEW, name);
	ni->type = it;
	ni->columns = list_new(tr->sa, (fdestroy) NULL);
	ni->t = t;
	ni->key = NULL;
...
{
        //mlog( LOG_DEBUG5,0,0,"sql_session_create");
	sql_session *s;
	...
	s->active = 0;
        s->gp_flag = 0;
	s->stk = stk;
        //mlog( LOG_DEBUG5,0,0,"#sql_session_reset ac=%d \n",ac );

	sql_session_reset(s, ac);
        //sql_session_reset(s,0 ) ;
	nr_sessions++;
	return s;
}
...
sql_trans_begin(sql_session *s)
{
  ...
  int i =0;
	...
	assert(tr->trans_state == TRANS_DEFAULT||tr->trans_state == TRANS_END );
	tr->trans_state = TRANS_START;
	for( i=0; i<TRANS_TABLE_COUNT; i++)
	{
	   tr->tbase_id_indx[i] = -1;
	}
	s->active = 1;
...
sql_trans_end(sql_session *s)
{
	...
	for( i=0; i<TRANS_TABLE_COUNT; i++ )
	{
		if(tr->tbase_id_indx[i] != -1)
		{
			ind = tr->tbase_id_indx[i];
			MT_lock_set(&tbase_lock);
			tbase_id[ind] = 0;
			mlog(LOG_DTM_DEBUG, JKD_TRANSACTION,0 , "tbase_id[%d] = %d" ,ind ,tbase_id[i]  );
			MT_lock_unset(&tbase_lock);
	...
	s->tr->trans_state = TRANS_END;
}

int
sql_trans_prepare(sql_trans *tr , int redo_log_off )
{
	int ok = LOG_OK;
	...
	if (bs_debug)
	 	fprintf(stderr, "#done prepare forwarding changes %d,%d\n", gtrans->stime, gtrans->wstime);
   
	MT_lock_unset(&tran_lock);
	 
	// set trans state
	if(ok == LOG_OK) {
		tr->trans_state = TRANS_PREPARED;
	} else {
		tr->trans_state = TRANS_ABORT;	
	}
	return (ok==LOG_OK)?SQL_OK:SQL_ERR;
...
int
sql_trans_commit_prepare(sql_trans *tr, int redo_log_off )
{
	int ok = LOG_OK;
	...
	if (bs_debug)
	 	fprintf(stderr, "#done commit forwarding changes %d,%d\n", gtrans->stime, gtrans->wstime);
   
	MT_lock_unset(&tran_lock);
	// set trans state
	if(ok == LOG_OK) {
	 	tr->trans_state = TRANS_END/*TRANS_COMMITED*/;
	} else {
		tr->trans_state = TRANS_ABORT;
	}
	return (ok==LOG_OK)?SQL_OK:SQL_ERR;
...
/* for retry_commit inserts and deletes */
void
tr_bat_insert(sql_trans *tr, logaction *la)
{
	node *n,*n1,*n2,*n3;
	sql_schema *s = NULL;
	sql_table *t = NULL;
	...
	if( strncmp( "D_", la->name, 2 ) != 0 )//for insert
	{
	  for( n= tr->schemas.set->h ;n; n=n->next )
	  {
	  	 s=n->data;		 	
	  	 if( strncmp( s->base.name, la->name, strlen(s->base.name) ) != 0 )
	  	 	continue;
	  	 if( strncmp( "_", la->name+ strlen(s->base.name), 1 ) != 0 )
		 	  continue;
	  	 for( n1= s->tables.set->h; n1;n1=n1->next )
	  	 {
	  	 	  t=n1->data;
	  	 	  if( strncmp( t->base.name, la->name+strlen(s->base.name)+1, strlen(t->base.name) ) != 0 )
	  	 	    continue;
	  	 	  dbat= t->data;
	  	 	  for( n2= t->columns.set->h; n2; n2=n2->next )
	  	 	  {
	  	 	  	c=n2->data;
	  	 	  	if( strcmp( c->base.name, la->name+strlen(s->base.name)+1+strlen(t->base.name)+1 ) != 0 )
	  	 	      continue;
	  	 	  	if(!c->data || !c->base.allocated )//����һ��sql_delta
	  	 	  	{
	  	       ...
	  	       (void)dup_bat(tr, c->t, obat, bat, type, isNew(oc), c->base.flag == TR_NEW); 
	  	        c->base.allocated = 1; t->base.allocated = 1;
	  	        
	  	 	  	}
	  	 	  	d=c->data;
	  	 	  
	  	 	  	d->wtime = c->base.wtime = c->t->base.wtime = c->t->s->base.wtime = tr->wtime = tr->wstime;
	          c->base.rtime = c->t->base.rtime = c->t->s->base.rtime = tr->rtime = tr->stime;
	  	 	  	if( strcmp( d->name, la->name) == 0 )
	  	 	  	{
	  	 	  		if( la->b )
	  	 	  		{
	  	 	  			temp_dup(la->b->batCacheid);
	  	 	  		  d->ibid = la->b->batCacheid;
	  	 	  		  d->ibase = d->cnt;
	  	 	  		  d->cnt += la->b->S->count;
	  	 	  	  }
	  	 	  	}
	...
	else// for delete
	{
	  ...
	  {
	  	 s=n->data; 
	  	 if( strncmp( s->base.name, la->name+2, strlen(s->base.name) ) != 0 )
	  	 	continue;
	  	 for( n1= s->tables.set->h; n1;n1=n1->next )
	  	 {
	  	 	  t=n1->data; 
	  	 	  if( strcmp( t->base.name, la->name+2+strlen(s->base.name)+1/*, strlen(t->base.name)*/ ) != 0 )
	  	 	    continue;
	  	 	  ...
	  	 	  if( !t->data || !t->base.allocated )
	  	 	  {
	  	 	  	 ...
	  	 	  	 dbat = t->data = ZNEW(sql_dbat); sql_dbat *obat = timestamp_dbat(ot->data, tr->stime);
	  	       (void)dup_dbat(tr, obat, dbat, isNew(ot), isTempTable(t)); 
	      	   t->base.allocated = 1;
	      	   
	      	}
	      	
	      	assert( strcmp( dbat->name, la->name) == 0 );
	  	 	  {
	  	 	  		if(la->b)
	  	 	  		{	  	 	  			
	  	 	  			BAT *db = NULL;
	      	      ...
	      	      temp_dup(la->b->batCacheid);
	      	      temp_dup(dbat->dbid);
	              db = temp_descriptor(dbat->dbid);              
	              BATappend(db, la->b, TRUE);
	              //BATcommit(db);
	              temp_destroy(dbat->dbid);
	  	 	  			/*temp_dup(la->b->batCacheid);
	  	 	  		  dbat->dbid = la->b->batCacheid;//��Ҫmerge�������Ǹ�ֵ ����Ҫ��*/
	  	 	  		  dbat->cnt += la->b->S->count;
	  	 	  		  dbat->wtime = tr->wtime; 
	  	 	  		  t->base.wtime = t->s->base.wtime = tr->wtime;
	  	 	  		  t->base.rtime = t->s->base.rtime = tr->rtime;
	  	 	  	  }
	  	 	  }
	  	 	  return;
	  	 }  
...
/* for retry_commit update */
void
tr_bat_update(sql_trans *tr, logaction *la)
{
	//la->name;
	node *n,*n1,*n2,*n3;
	sql_schema *s = NULL;
	...
	for( n= tr->schemas.set->h ;n; n=n->next )
	{
		 ...
		 for( n1= s->tables.set->h; n1;n1=n1->next )
		 {
		 	  ...
		 	  for( n2= t->columns.set->h; n2; n2=n2->next )
		 	  {
		 	  	...
		 	  	if( strcmp( c->base.name, la->name+strlen(s->base.name)+1+strlen(t->base.name)+1 ) != 0 )
		 	      continue;
		 	  	if(!c->data || !c->base.allocated )//TODO������һ��sql_delta
		 	  	{
		 	  		int type = c->type.type->localtype;
		 	  		sql_column *oc = tr_find_column(tr->parent, c);
		        sql_delta *bat = c->data = ZNEW(sql_delta), *obat = timestamp_delta(oc->data, tr->stime);
	        ...
	        c->base.rtime = c->t->base.rtime = c->t->s->base.rtime = tr->rtime = tr->stime;
		 	  	if( strcmp( d->name, la->name) == 0 )
		 	  	{
		 	  		...
		 	  		{
		 	  			temp_dup(la->b->batCacheid);
		 	  		  d->uvbid = la->b->batCacheid;
		 	  		  temp_dup(la->uid->batCacheid);
		 	  		  d->uibid = la->uid->batCacheid;
		 	  		 // d->ibase = d->cnt;
		 	  		  d->ucnt += la->b->S->count;
		 	  	  }
		 	  	}
...
void
tr_bat_create_indx(sql_trans *tr,trans *rtr, logaction *la)
{//{type = 6, nr = 0, ht = -1, tt = 7, id = 0, name = 0x6b833f0 "public_test152@test152_indx", b = 0x0, uid = 0x0}
	
	node *n,*n1,*n2,*n3,*m,*m1;
	sql_schema *s,*os = NULL;
	sql_table *t = NULL;
	sql_column *c = NULL;
	sql_delta *d = NULL;
	sql_dbat *dbat = NULL;
	...
	{
		 s=n->data; 
		 if( strncmp( s->base.name, la->name, strlen(s->base.name) ) != 0 )/* find schema_name*/
		 	continue;
		 if( strncmp( "_", la->name+ strlen(s->base.name), 1 ) != 0 )
		 	continue;
		 for( n1= s->tables.set->h; n1;n1=n1->next )
	   {
	  	 	  ...
	  	 	  if( strncmp( t->base.name, la->name+strlen(s->base.name)+1, strlen(t->base.name) ) != 0 )
	  	 	    continue;
		 		  if( strncmp( "@", la->name+ strlen(s->base.name)+1+strlen(t->base.name), 1 ) != 0 )
		 	      continue;
		 	    
		 	    strcpy( iname, la->name+ strlen(s->base.name)+1+strlen(t->base.name)+1);/* got index_name */		 
		 	    break;	    
		 }
		 /* find index in trt->cr_indx */
		 create_indx *tmp = rtr->cr_indx;
		 while( tmp )// find create_tab
		 {
		 			 if( strcmp(tmp->iname,iname ) == 0 )
		 			 {
		 			 	  //Ȼ����֤index��table�Բ���,��֤��һ���ַ��ǲ���'_',//�ԵĻ���break;
		 			 	  if( tmp->toid == t->base.id )
		 			 	  {
		 			 	  	  break;
              }
		 			 }
		 ...
		 create_dependency *tmpd = rtr->cr_depds;
		 while(tmpd)
		 {
		 	  if( tmpd->depend_id == tmp->ioid )
		 	  	break;
		 	  else
		 	    tmpd = tmpd->next;
		 }
		 //sql_idx * i = sql_trans_create_idx_with_id( tr, t, iname, tmp->ioid ,tmp->type );
		 sql_idx *ni = SA_ZNEW(tr->sa, sql_idx);
	   sql_schema *syss = find_sql_schema(tr, isGlobal(t)?"sys":"tmp");
     sql_table *sysidx = find_sql_table(syss, "idxs");

   	 assert(iname);
     base_init(tr->sa, &ni->base, tmp->ioid, TR_NEW, iname);
	   ni->type = tmp->type;
	   ni->columns = list_new(tr->sa, (fdestroy) NULL);
	   ni->t = t;
	   ni->key = NULL;

     //sql_trans_create_dependency(tr, tmpd->doid , tmpd->depend_id , tmpd->depend_type );
	  ...
	  /* if (!isDeclaredTable(t))
	   ...
	   if (isGlobal(t)) 
		 ...
		 if (isTable(ni->t) && idx_has_column(ni->type)) {
		 int p = (tr->parent == gtrans && !isTempTable(ni->t));

		 if ((p  && store_funcs.snapshot_create_idx(tr, ni) != LOG_OK) ||
		    (p  && store_funcs.log_create_idx(tr, ni) != LOG_OK))
		    return ;
		 }   
		    return;
	}
...
void
tr_bat_create(sql_trans *tr,trans *rtr, logaction *la)
{
  //create:tr->changes[18] :6, -1, 5, public_ctestpp_a ,NULL
  //create:tr->changes[19] :6, -1, 5, public_ctestpp_b ,NULL
  //create:tr->changes[20] :6, -1, 6, D_public_ctestpp ,NULL
	node *n,*n1,*n2,*n3,*m,*m1;
	sql_schema *s,*os = NULL;
	sql_table *t = NULL;
	sql_column *c = NULL;
	...
	if( strstr( la->name, "@" ) != NULL && la->tt == TYPE_wrd )	/*if it is a index*/
	{
		 		 tr_bat_create_indx(tr,  rtr, la);
		 		 return;
	}

	//��Ҫ����sql_table��sql_dbat��sql_column��sql_delta
	if( strncmp( "D_", la->name, 2 ) == 0 )return;
	for( n= tr->schemas.set->h ,m= gtrans->schemas.set->h; n; n=n->next,m=m->next )
	{
		 s=n->data; os=m->data;
		 if( strncmp( s->base.name, la->name, strlen(s->base.name) ) != 0 )/* find schema_name*/
		 	continue;
		 	if( strncmp( "_", la->name+ strlen(s->base.name), 1 ) != 0 )
		 		...
		 		//find table_name ,toid,column_name ,coid etc.
		 		create_tab *tmp = rtr->cr_tabs;
		 		create_clm *tmpc = rtr->cr_cols;        
        
		 		while( tmp )// find create_tab
		 		{
		 			 if( strncmp(tmp->tname, la->name+ strlen(s->base.name)+1, strlen(tmp->tname)< strlen(la->name)-strlen(s->base.name)-1? strlen(tmp->tname):-1)
		 			 	== 0 )
		 			 {
		 			 	  //Ȼ����֤schema�Բ���,��֤��һ���ַ��ǲ���'_',//�ԵĻ���break;
		 			 	  if( tmp->soid == s->base.id )
		 			 	  {
		 			 	  	if( strncmp( "_", la->name+ strlen(s->base.name), 1 ) == 0 )
		 			 	  	{
		 			 	  		strcpy(tname, tmp->tname );
		 			 	  	  break;
		 			 	  	}
              }
		 			 }
		 		...
		 		while( tmpc )//find create_clm
		 		{
		 			 if( strcmp(tmpc->cname, la->name+ strlen(s->base.name)+1+ strlen(tmp->tname)+1)
		 			 	== 0 )
		 			 {
		 			 	  //��֤toid�Ƿ�Ե���
		 			 	  //�Ե��ϣ�break��
		 			 	  if( tmpc->toid == tmp->toid )
		 			 	  {
		 			 	  		strcpy(cname, tmpc->cname );
		 			 	  	  break;		 			 	  
              }
		 			 }
		 			 tmpc = tmpc->next;
		 		...
		 		if( t == NULL )	
		 		{
		      t = create_sql_table_with_id(tr->sa, tmp->toid, tname, tmp->type, tmp->system, /*persistence*/0 ,tmp->commit_action );
		    
		  	  sql_schema *syss = find_sql_schema(tr, isGlobal(t)?"sys":"tmp");
	        sql_table *systable = find_sql_table(syss, "_tables");
	        sql_table *syscolumn = find_sql_table(syss, "_columns");
		    
		      t->query = tmp->query ? sa_strdup(tr->sa, tmp->query) : NULL;
	        t->s = s;
    	    t->sz = sz;
    	    if (sz < 0)
	   	    t->sz = COLSIZE;
    	    ...
    	    if (isTable(t)) {
    	    	if (store_funcs.create_del(tr, t) != LOG_OK) {     //sql_dbat
    	    		if (bs_debug)
    	    			fprintf(stderr, "#\tload table %s missing 'deletes'", t->base.name);
    	    		t->persistence = SQL_GLOBAL_TEMP;
    	   	  }
    	...
    	if (sql_trans_name_conflict(tr, t->s->base.name, t->base.name, cname))
		     return ;
		  sql_subtype tpe;   
		  if (!sql_find_subtype(&tpe, tmpc->type, tmpc->type_digits, tmpc->type_scale))  //get tpe using tmpc type info
		      return ;
		  sql_column *col =  create_sql_column_with_id(tr->sa, tmpc->coid, t, cname, &tpe );// oid do not use next_oid()
		  if (isTable(col->t))
	    	if (store_funcs.create_col(tr, col) == LOG_ERR)
			    return ;
			col->base.wtime = t->base.wtime = t->s->base.wtime = tr->wtime = tr->wstime;
	   if (tpe.type->s) /* column depends on type */
		 sql_trans_create_dependency(tr, tpe.type->base.id, col->base.id, TYPE_DEPENDENCY);
		 col->base.allocated = 1; t->base.allocated = 1;
		 if( store_funcs.snapshot_create_col(tr, col) != LOG_OK )return;
		 if( store_funcs.log_create_col(tr, col) != LOG_OK )return;
		 return;
	}
}
void
tr_bat_clear(sql_trans *tr, logaction *la)
{
	//CLEAR:tr->changes[0] :9, D_public_ttt 
  //CLEAR:tr->changes[1] :9, public_ttt_a 
  //CLEAR:tr->changes[2] :9, public_ttt_b 
	node *n,*n1,*n2,*n3;
	sql_schema *s = NULL;
	sql_table *t = NULL;
	sql_column *c = NULL;
	...
	if( strncmp( "D_", la->name, 2 ) == 0 )//CLEAR:tr->changes[0] :9, D_public_ttt ����sql_dbat
	{
	  for( n= tr->schemas.set->h ; n; n=n->next )
	  {
	  	 s=n->data; 
	...
	else //CLEAR:tr->changes[1] :9, public_ttt_a ����sql_delta
	{
		for( n= tr->schemas.set->h ;n; n=n->next )
	  {
	  	 s=n->data;		 	
...
void
tr_bat_drop_indx(sql_trans *tr,trans *rtr, logaction *la)
{
	node *n,*n1,*n2,*n3,*m,*m1;
	sql_schema *s,*os = NULL;
	sql_table *t = NULL;
	...
	node *nn = list_find_name(s->idxs, iname);
	sql_idx *i;
	if (!nn)
		return;
	i = nn->data;
	sql_trans_drop_idx( tr, s, i->base.id, 0 );
	store_funcs.log_destroy_idx(tr, i);
	return;
}

void
tr_bat_destroy(sql_trans *tr,trans *rtr, logaction *la)
{
	//  update:tr->changes[0] :3, 0, 6, D_sys__tables ,NULL
  //  update:tr->changes[1] :3, 0, 6, D_sys__columns ,NULL
  //  DESTROY:tr->changes[2] :7, D_public_ttt 
  //  DESTROY:tr->changes[3] :7, public_ttt_a 
  
  // DESTROY index :tr->changes[0] :7, public_ttt@ttt_idx 
  node *n,*n1,*n2,*n3,*m,*m1;
	sql_schema *s,*os = NULL;
	sql_table *t = NULL;
	sql_column *c = NULL;
	...
	if( strstr( la->name, "@" ) != NULL )	/*if it is a index*/
	{
		 tr_bat_drop_indx(tr,  rtr, la);
		 return;
	}
	...
	mlog( LOG_DTM_DEBUG ,JKD_TRANSACTION, 0,"tr_bat_destroy t->base.id=%d", t->base.id  );
	sql_trans_drop_table (tr, s, t->base.id, 0);
	store_funcs.log_destroy_del(tr, t);
}

/*apply  logaction to sql_trans->sql_delta*/
void
la_apply_tr(sql_trans *tr, trans* rtr, logaction *c)
{
	int ok = LOG_OK;
	tr->wstime = tr->wtime = timestamp();
	
	switch (c->type) {
	case LOG_INSERT:   //3 inserts and deletes
		tr_bat_insert(tr, c);
		break;
	case LOG_UPDATE:  //5 update
		tr_bat_update(tr, c);
		break;
	case LOG_CREATE://6  create
		tr_bat_create(tr,rtr, c);
		break;
	/*case LOG_USE: //8
		tr_bat_use(tr, c);
		break;*/
	case LOG_DESTROY://7  drop
		tr_bat_destroy(tr,rtr, c);
		break;
	case LOG_CLEAR:  //9 ; delete * from t;
		tr_bat_clear(tr, c);
	}
	//lg->changes++;???
}
...
int
sql_trans_retry_commit_prepare(sql_trans *tr, int redo_log_off )
{
	 int ok = LOG_OK;
   int i;
	 /* write phase */
	 if (bs_debug)
	 	fprintf(stderr, "#forwarding changes %d,%d %d,%d\n", gtrans->stime, tr->stime, gtrans->wstime, tr->wstime);
    tr->wtime = tr->wstime = tr->rtime = tr->stime = timestamp();
   trans *rtr = recover_tr;
    while( rtr )
    {
    	 	for ( i = 0; i < rtr->nr; i++) {
		      la_apply_tr(tr, rtr, &rtr->changes[i]);		      
      	}
      	if( rtr->cr_schema != NULL)
		      	sql_trans_create_schema_with_id( tr, rtr->cr_schema->soid, rtr->cr_schema->sname, rtr->cr_schema->authorization, rtr->cr_schema->owner);
    	  rtr = rtr->tr;
    }
	...
	if (bs_debug)
		fprintf(stderr, "#forwarding changes %d,%d %d,%d\n", gtrans->stime, tr->stime, gtrans->wstime, tr->wstime);
	/* snap shots should be saved first */
	//strcpy( GlobalLoggerGid, tr->gtid );
	if (tr->parent == gtrans) {

		if (ok == LOG_OK)
			ok = logger_funcs.log_retry_end();
		tr->schema_number = store_schema_number();
	}
	if (ok == LOG_OK) {
		/* It is save to rollforward the changes now. In case 
		...
		if (ok == LOG_OK)
			ok = logger_funcs.restart();
	
	rtr = recover_tr;
  while( rtr )
  {
    	 	for ( i = 0; i < rtr->nr; i++) {
		       la_destroy(&rtr->changes[i]);
      	}
    	  tr_destroy(rtr);
    	 rtr = rtr->tr;
  }
...
int
sql_trans_abort_prepare(sql_trans *tr)
{
	int ok = LOG_OK;
	assert(tr->trans_state == TRANS_PREPARED);
	
	// not consider subtran here
	// set trans state
	tr->trans_state = TRANS_ABORT;
	return (ok==LOG_OK)?SQL_OK:SQL_ERR;
}
...
int
sql_trans_retry_abort_prepare(sql_trans *tr)
{
	int ok = LOG_OK;
	...
	if (ok == LOG_OK)
		ok = logger_funcs.log_retry_abort();
	 		
	 //TODO:retry abort prepared		
	 MT_lock_unset(&bs_lock);			

	 if (ok < 0) {
		GDKfatal("write-ahead log store re-load failure");
	 }
	 store_nr_active--;
	 // set trans state
	 if(ok == LOG_OK) {
...
sql_table *
trans_rename_table( sql_trans *tr, const char *sname, const char *tname, const char *tnewname )
{
	int i = 0;
	sql_table * t = NULL;
	sql_trans *tr1  = NULL;
	t = rename_sql_table(tr, sname, tname, tnewname );
	t = rename_sql_table(gtrans, sname, tname, tnewname );
	
	for( i=0; i<MAX_SPARES; i++ )
	{
		tr1=spare_trans[i];
		if( tr1 )
	  t = rename_sql_table(tr1, sname, tname, tnewname );
	}
