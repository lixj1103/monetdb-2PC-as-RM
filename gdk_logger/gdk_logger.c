...
typedef struct logformat_t {
	...
	int tid;
	lng nr;
	char gtid[TMGIDSIZE]; /* global trans id; add for distributed transation */
	char coltype; /* column type :str , int, long etc. for future use*/
} logformat;
...
trans *recover_tr = NULL;
//create_tab *cr_tabs = NULL;
//create_clm *cr_cols = NULL;
...
{
	int rc;
	rc = ( mnstr_read(l->log, &data->flag, 1, 1) == 1 &&
		mnstr_readLng(l->log, &data->nr) == 1 &&
		mnstr_readInt(l->log, &data->tid) == 1 );
#ifdef  MONETDB_DTX		
		if( rc == 1  )
		{
			char *gtid = log_read_string(l);
			if (!gtid) {
				strncpy(data->gtid, "0000000000-0000000000", TMGIDSIZE);
			} else {
				strncpy(data->gtid, gtid, TMGIDSIZE);
			}
		}
#endif	
		return rc;
...
log_read_clear(logger *lg, trans *tr, char *name)
{
	...
	if (tr_grow(tr)) {
		...
		mlog( LOG_INFO,JKD_TRANSACTION, 0, "CLEAR:tr->changes[%d] :%d, %s ",tr->nr,
			  tr->changes[tr->nr].type,tr->changes[tr->nr].name  );
		tr->nr++;
	}
}
...
static int log_read_seq2(logger *lg, logformat *l)
{
        int seq = (int) l->nr;
        lng val;
        BUN p;
...
#endif
      //for create  ;add something to remember the oid and table_name col etc.
        if( strcmp( name, "sys__tables_id" ) == 0 ) /* init create_tab */
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   if( tr->cr_tabs == NULL ) 
      	   tmp = tr->cr_tabs = (create_tab *) malloc (sizeof(create_tab));
      	   ...
      	   else 
      	   {
      	   	 while( tmp->next )
      	     {
      	      ...
      	      tmp->next = (create_tab *) malloc (sizeof(create_tab));
      	      tmp = tmp->next;
      	   }
           tmp->toid = *(int*)t;
           tmp->soid = 0;
           memset(tmp->tname,0,sizeof(tmp->tname));
           tmp->type = -1;
           tmp->access = -1;
           tmp->system = -1;
           tmp->commit_action = -1;
        ...
        if( strcmp( name, "sys__tables_name" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
      	   	 if( strlen(tmp->tname) == 0 )
      	   	 {
      	   	 	  strncpy(tmp->tname , (str)t ,strlen((str)t));
      	   	 	  break;
      	   	 }
      	   	 tmp = tmp->next;
      	   }
        }
        //sys__tables_schema_id
        if( strcmp( name, "sys__tables_schema_id" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
        ...
        if( strcmp( name, "sys__tables_query" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
      	   	 if( tmp->query == NULL )
      	   	 {
      	   	 	  tmp->query = (char *) malloc (strlen((str)t));
      	   	 	  strncpy(tmp->query , (str)t ,strlen((str)t));
      	   	 	  break;
      	   	 }
      	   	 tmp = tmp->next;
      	   }
        }
        if( strcmp( name, "sys__tables_type" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
        ...
        if( strcmp( name, "sys__tables_access" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
        ...
        if( strcmp( name, "sys__tables_system" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
        ...
        if( strcmp( name, "sys__tables_commit_action" ) == 0 )
        {
      	   create_tab * tmp = tr->cr_tabs ;
      	   while( tmp )
      	   {
      	   	 if( tmp->commit_action == -1 )
      	   	 {
      	   	 	  tmp->commit_action = *(int*)t;
       ...
       /* //view query 
        ...
        if( strcmp( name, "sys__columns_id" ) == 0 ) /* init create_clm*/
        {
      	   create_clm * tmpc = tr->cr_cols ;
      	   if(  tr->cr_cols == NULL )
      	   {
      	   	 tr->cr_cols	= (create_clm *) malloc (sizeof(create_clm));
      	     tmpc = tr->cr_cols;
      	   }
      	   else 
      	   {
      	   	 while( tmpc->next )
      	     {
      	      ...
      	      tmpc->next = (create_clm *) malloc (sizeof(create_clm));
      	      tmpc = tmpc->next;
      	   }
           tmpc->coid = *(int*)t;
           memset(tmpc->cname , 0 ,sizeof(tmpc->cname));
           memset(tmpc->type , 0 ,sizeof(tmpc->type));
           tmpc->toid = 0;
           tmpc->type_digits = -1;
           tmpc->type_scale = -1;
           tmpc->next = NULL;
        }
        if( strcmp( name, "sys__columns_name" ) == 0 )
        {
      	   create_clm * tmp = tr->cr_cols ;
      	   while( tmp )
      	   {
      	   	 if( strlen(tmp->cname) == 0 )
      	   	 {
      	   	 	  strcpy( tmp->cname , t);
      	   	 	  break;
      	   	 }
        ...
        if( strcmp( name, "sys__columns_type" ) == 0 )
        {
      	   create_clm * tmp = tr->cr_cols ;
      	   while( tmp )
      	   {
      	   	 if( strlen(tmp->type) == 0 )
      	   	 {
      	   	 	  strcpy( tmp->type , t);
      	   	 	  break;
      	   	 }
        ...
        if( strcmp( name, "sys__columns_type_digits" ) == 0 )
        {
      	   create_clm * tmp = tr->cr_cols ;
      	   while( tmp )
      	   {
      	   	 if( tmp->type_digits == -1 )
      	   	 {
      	   	 	  tmp->type_digits =  *(int*)t ;
      	   	 	  break;
      	   	 }
      	   	 tmp = tmp->next;
      	   }
        ...
        //tmpc->type_scale
        if( strcmp( name, "sys__columns_type_scale" ) == 0 )
        {
      	   create_clm * tmp = tr->cr_cols ;
      	   while( tmp )
      	   {
      	   	 if( tmp->type_scale == -1 )
      	   	 {
      	   	 	  tmp->type_scale =  *(int*)t ;
      	   	 	  break;
      	   	 }
      	   	 tmp = tmp->next;
      	   }
        }
        //sys__columns_table_id  
        if( strcmp( name, "sys__columns_table_id" ) == 0 )
        {
      	   create_clm *tmp_pre, *tmp = tr->cr_cols;
      	   tmp_pre = tr->cr_cols;
      	   while( tmp )
      	   {
      	      	 if( tmp->toid == 0 )
        ...
        // add for create dependency
        if( strcmp( name, "sys_dependencies_id" ) == 0 ) /* init create_clm*/
        {
      	   create_dependency * tmpd = tr->cr_depds ;
      	   if(  tr->cr_depds == NULL )
      	   {
      	   	 tr->cr_depds	= (create_dependency *) malloc (sizeof(create_dependency));
      	     tmpd = tr->cr_depds;
      	   }
      	   else 
      	   {
      	   	 while( tmpd->next )
      	     {
      	      ...
      	      tmpd->next = (create_dependency *) malloc (sizeof(create_dependency));
      	      tmpd = tmpd->next;
      	   }
           tmpd->doid = *(int*)t;
           tmpd->depend_id = -1 ;
        ...
        if( strcmp( name, "sys_dependencies_depend_id" ) == 0 )
        {
      	   create_dependency * tmp = tr->cr_depds ;
      	   while( tmp )
      	   {
      	   	 if( tmp->depend_id == -1 )
      	   	 {
        ...
        if( strcmp( name, "sys_dependencies_depend_type" ) == 0 )
        {
      	   create_dependency * tmp = tr->cr_depds ;
      	   while( tmp )
      	   {
      	   	 if( tmp->depend_type == -1 )
      	   	 {
      	   	 	  tmp->depend_type = *(short*)t;
        ...
        // add for create index
        if( strcmp( name, "sys_idxs_id" ) == 0 ) /* init create_clm*/
        {
      	   create_indx * tmpi = tr->cr_indx ;
      	   if(  tr->cr_indx == NULL )
      	   {
      	   	 tr->cr_indx	= (create_indx *) malloc (sizeof(create_indx));
      	     tmpi = tr->cr_indx;
      	   }
      	   else 
      	   {
      	   	 while( tmpi->next )
      	     {
      	      ...
      	      tmpi->next = (create_indx *) malloc (sizeof(create_indx));
      	      tmpi = tmpi->next;
      	   }
           tmpi->ioid = *(int*)t;
           memset(tmpi->iname , 0 ,sizeof(tmpi->iname));
           tmpi->type = -1 ;
           tmpi->toid = 0;
           tmpi->next = NULL;
        }
        if( strcmp( name, "sys_idxs_table_id" ) == 0 )
        {
      	   create_indx *tmp_pre, *tmp = tr->cr_indx;
      	   tmp_pre = tr->cr_indx;
      	   while( tmp )
      	   {
      	      	 if( tmp->toid == 0 )
        ...
        if( strcmp( name, "sys_idxs_type" ) == 0 )
        {
      	   create_indx * tmp = tr->cr_indx ;
      	   while( tmp )
      	   {
      	   	 if( tmp->type == -1 )
      	   	 {
        ...
        if( strcmp( name, "sys_idxs_name" ) == 0 )
        {
      	   create_indx * tmp = tr->cr_indx ;
      	   while( tmp )
      	   {
      	   	 if( strlen(tmp->iname) == 0 )
      	   	 {
      	   	 	  strcpy( tmp->iname , t);
      	   	 	  break;
      	   	 }
        ...
        // add for create schema
        if( strcmp( name, "sys_schemas_id" ) == 0 ) /* init create_clm*/
        {
      	   create_schema * tmps = tr->cr_schema;
      	   if(  tr->cr_schema == NULL )
      	   {
      	   	 tr->cr_schema	= (create_schema *) malloc (sizeof(create_schema));
      	     tmps = tr->cr_schema;
      	   }
      	   else 
      	   {
      	   	 while( tmps->next )
      	     {
      	      ...
      	      tmps->next = (create_schema *) malloc (sizeof(create_schema));
      	      tmps = tmps->next;
      	   }
           tmps->soid = *(int*)t;
           memset(tmps->sname , 0 ,sizeof(tmps->sname));
           tmps->authorization = -1;
           tmps->authorization = -1;
           tmps->system = FALSE;
           tmps->next = NULL;
        }
        
        if( strcmp( name, "sys_schemas_name" ) == 0 )
        {
      	   create_schema * tmp = tr->cr_schema ;
      	   while( tmp )
      	   {
        ...
        if( strcmp( name, "sys_schemas_authorization" ) == 0 )
        {
      	   create_schema *tmp_pre, *tmp = tr->cr_schema;
      	   tmp_pre = tr->cr_schema;
      	   while( tmp )
      	   {
      	      	 if( tmp->authorization == 0 )
      	      	 {
      	      	 	  tmp->authorization = *(int*)t;  
      	      	 	  break;    	   	 	  
      	      	 }
      	      	 tmp_pre = tmp;
      	      	 tmp = tmp->next;
        ...
        //for drop ; 
        // D_sys__tables  D_sys__columns
         // drop_tab *dr_tabs;
         // drop_col *dr_cols;
        if( strcmp( name, "D_sys__tables" ) == 0 )
        {
         	drop_tab* tmp = tr->dr_tabs;
         	if(  tr->dr_tabs == NULL )
      	   {
      	   	 tr->dr_tabs	= (drop_tab *) malloc (sizeof(drop_tab));
      	     tmp = tr->dr_tabs;
      	   }
      	   else 
      	   {
      	   	 while( tmp->next )
      	     {
      	      ...
      	      tmp->next = (drop_tab *) malloc (sizeof(drop_tab));
      	      tmp = tmp->next;
      	   }
           tmp->toid = *(int*)t;
           tmp->next = NULL;
        }
        if( strcmp( name, "D_sys__columns" ) == 0 )
        {
        	drop_col* tmp = tr->dr_cols;
         	if(  tr->dr_cols == NULL )
      	   {
      	   	 tr->dr_cols	= (drop_col *) malloc (sizeof(drop_col));
      	     tmp = tr->dr_cols;
      	   }
      	   else 
      	   {
      	   	 while( tmp->next )
      	     {
      	      ...
      	      tmp->next = (drop_col *) malloc (sizeof(drop_col));
      	      tmp = tmp->next;
      	   }
           tmp->coid = *(int*)t;
           tmp->next = NULL;
		...
		if (tr_grow(tr)) {
			...
			mlog( LOG_INFO,JKD_TRANSACTION, 0, "update:tr->changes[%d] :%d, %d, %d, %s ,NULL",tr->nr,
			  tr->changes[tr->nr].type,tr->changes[tr->nr].ht,tr->changes[tr->nr].tt ,tr->changes[tr->nr].name  );
			tr->nr++;
		}
	} else {
...
log_read_destroy(logger *lg, trans *tr, char *name)
{
	...
	if (tr_grow(tr)) {
		...
		tr->changes[tr->nr].name = GDKstrdup(name);
		mlog( LOG_INFO,JKD_TRANSACTION, 0, "DESTROY:tr->changes[%d] :%d, %s ",tr->nr,
			  tr->changes[tr->nr].type,tr->changes[tr->nr].name  );
		tr->nr++;
	}
}
...
static int
log_read_create(logger *lg, trans *tr, char *name)//aad �ڴ˺�������Ҫʵ��column��table�Ĺ���
{
	...
	if (!buf) {
		...
		*ta = 0;
		...
		if (tr_grow(tr)) {
			...
			tr->changes[tr->nr].ht = ht;
			tr->changes[tr->nr].tt = tt;
			tr->changes[tr->nr].name = GDKstrdup(name);//public_ctestpp_a 
			tr->changes[tr->nr].b = NULL;
			mlog( LOG_INFO,JKD_TRANSACTION, 0, "create:tr->changes[%d] :%d, %d, %d, %s ,NULL",tr->nr,
			  tr->changes[tr->nr].type,tr->changes[tr->nr].ht,tr->changes[tr->nr].tt ,tr->changes[tr->nr].name  );
...
log_read_use(logger *lg, trans *tr, logformat *l, char *name)
{
	...
	if (tr_grow(tr)) {
		...
		tr->changes[tr->nr].name = GDKstrdup(name);
		tr->changes[tr->nr].b = NULL;
		mlog( LOG_INFO,JKD_TRANSACTION, 0, "USE:tr->changes[%d] :%d, %d, %s ,NULL",tr->nr,
			  tr->changes[tr->nr].type,tr->changes[tr->nr].nr  ,tr->changes[tr->nr].name  );
		tr->nr++;
	}
}
...
static trans *
tr_create(trans *tr, int tid, char *gid )
{
...
tr_commit(logger *lg, trans *tr)
{
	...
	/* TODO:here are some inconsistent tr->nr > 0 while changes is null */
	if (tr->changes) {
		for (i = 0; i < tr->nr; i++) {
...
/*static trans *
tr_prepare(logger *lg, trans *tr)
{
	if (lg->debug & 1)
		fprintf(stderr, "#tr_prepare\n");
	return tr;
}*/
...
logger_retry_open(logger *lg)
{
	...
	lg->log = append_wstream(filename);
	lg->end = 0;
...
/* get gid status from pm_clog*/
int
gp_distributed_xid_status( char *gid )
{
	DIR * p_dir;
  ...
  while( p_dirent= readdir( p_dir ) )
  {
    ...
    fp =fopen( fileName,"r+" ); 
      ...
      {	
      	if( buf[TMGIDSIZE-1] == 'C') /* 'C' was written by template1 covering 'P' when recovery */
      	{
      			ret = RECOVER_COMMITED;
        }
        else if( buf[TMGIDSIZE-1] == 'A')/* 'A' was written by template1 covering 'P' when recovery */
      	{
      			ret = RECOVER_ABORT;
        }
        else if( buf[TMGIDSIZE-1] == 'P')/* 'P' was written by db(p_dirent->d_name) when mvc_prepare */
      	{
      			ret = RETRY_PREPARED;
        }
      }
...
{//sleep(20);
	...
	while (!err && log_read_format(lg, &l)) {
		...
		if ( l.flag != LOG_START && l.flag != LOG_END && l.flag != LOG_SEQ && l.flag != LOG_PREPARE ) {
		...
		if (l.flag != LOG_START)
			recover_tr = tr_find(recover_tr, l.tid);
		switch (l.flag) {
		case LOG_START:
			assert(l.nr <= (lng) INT_MAX);
			if (l.nr > lg->tid)
				lg->tid = (int)l.nr;
			recover_tr = tr_create(recover_tr, (int)l.nr, l.gtid );
			if (lg->debug & 1)
		...
		case LOG_END:
			if (recover_tr == NULL)
				...
				recover_tr = tr_abort(lg, recover_tr);
			else
				recover_tr = tr_commit(lg, recover_tr);
			break;
		/*case LOG_PREPARE:
      if (tr == NULL)
				err = 1;
		...
		case LOG_INSERT:
		case LOG_UPDATE:
			if (name == NULL || recover_tr == NULL)
				err = 1;
			else
				err = (log_read_updates(lg, recover_tr, &l, name) != LOG_OK);
			break;
		case LOG_CREATE:
			if (name == NULL || recover_tr == NULL)
				err = 1;
			else
				err = (log_read_create(lg, recover_tr, name) != LOG_OK);
			break;
		case LOG_USE:
			if (name == NULL || recover_tr == NULL)
				err = 1;
			else
				log_read_use(lg, recover_tr, &l, name);
			break;
		case LOG_DESTROY:
			if (name == NULL || recover_tr == NULL)
				err = 1;
			else
				log_read_destroy(lg, recover_tr, name);
			break;
		case LOG_CLEAR:
			if (name == NULL || recover_tr == NULL)
				err = 1;
			else
				log_read_clear(lg, recover_tr, name);
			break;
		default:
			err = -2;
		}
	...
	/* remaining transactions are not committed, we do it as recovery command */
	/* for gp segment,this should do as recovery command;  
	todo: for monetdb itself, should abort  */
	while (recover_tr)
	{   
		  int rflag = gp_distributed_xid_status(l.gtid); // get gid from pm_clog
			
			if( rflag == RECOVER_COMMITED )
			{
				  l.flag = LOG_END;
	        l.tid = lg->tid;
	        l.nr = l.tid;
			...
			else if( rflag == RETRY_PREPARED )
			{//retry commit prepared or retry abort prepared
				 //store_nr_active++;
				 lg->tid = recover_tr->tid;
				 IndoubtTransRetryFlag=1;
				 break;
			}/**/
			else //no prepare transaction,should abort  /* */
			{
				 recover_tr = tr_abort(lg, recover_tr);
			}
	}
...
logger_readlogs(logger *lg, FILE *fp, char *filename)
{
	...
	//memcpy( CurrLoggerGid, lg->gtid, TMGIDSIZE );/* add for distributed transation  */
	return res;
}
...
logger_create(int debug, const char *fn, const char *logdir, int version, preversionfix_fptr prefuncp, postversionfix_fptr postfuncp, int keep_persisted_log_files)
{
	...
	if( recover_tr != NULL)/*add for test*/
	{
		return lg;
	}
...
log_bat_persists(logger *lg, BAT *b, const char *name)
{
	...
	//memcpy(lg->gtid, '0000000000-0000000000', TMGIDSIZE-1);
	strncpy( l.gtid, QEDtxContextInfo.distributedId,TMGIDSIZE);
...
log_tstart(logger *lg)
{
	...
	if (!pbat_logger) {
		pbat_logger = (struct logger *) GDKmalloc(sizeof(struct logger));
		((logger *)pbat_logger)->tid = lg->tid;
	}
...
log_tend(logger *lg)
{
	...
	if (lg->debug & 1)
		fprintf(stderr, "#log_tend %d\n", pbat_logger?((logger *)pbat_logger)->tid:lg->tid);
	...
	if (!pbat_logger || ((logger *)pbat_logger)->tid <= 0) {
		l.tid = lg->tid;
		l.nr = lg->tid;
	} else {
		l.tid = ((logger *)pbat_logger)->tid;
		l.nr = ((logger *)pbat_logger)->tid;
		/* set to 0 */
		((logger *)pbat_logger)->tid = 0;
	}
...
/*
 * function log_retry_end is added for rety_commit_prepare
 */
int
log_retry_end(logger *lg)
{
	logformat l;
	gdk_return res = GDK_SUCCEED;
	if (lg->debug & 1)
		fprintf(stderr, "#log_retry_end %d\n", lg->tid);
  ...
  /* */if (DELTAdirty(lg->snapshots_bid)) {
	...
	if (logger_retry_open(lg) == LOG_ERR) {
		logger_destroy(lg);
		return -1;
	}
	l.flag = LOG_END ;
	...
	if (res != GDK_SUCCEED ||
	    ...
	    mnstr_fsync(lg->log) ||
	    pre_allocate(lg) != GDK_SUCCEED) {
		fprintf(stderr, "!ERROR: log_retry_end: write failed\n");
		return LOG_ERR;
	}

	lg->changes += recover_tr->nr ;
...
log_retry_abort(logger *lg)
{
	...
	if (log_write_format(lg, &l) == LOG_ERR ||
	    ...
	    mnstr_fsync(lg->log) ||
	    pre_allocate(lg) != GDK_SUCCEED) {
		fprintf(stderr, "!ERROR: log_retry_abort: write failed\n");
		return LOG_ERR;
	}
		fflush(stdout);

	if (logger_restart(lg) != LOG_OK ||
...
/* write prepare log; add for distributed transation*/
int
log_prepare(logger *lg)
{
	logformat l;
	gdk_return res = GDK_SUCCEED;

	if (lg->debug & 1)
		fprintf(stderr, "#log_prepare %d\n", lg->tid);
...
/*
 * this funtion add for logReader tool ;
 * read the log file and translate it to char
 * for log interpreter
 */
int
logger_readlog2(logger *lg, char *filename)
{
        trans *tr = NULL;
        logformat l;
        int err = 0;
  ...
  lg->debug = 1;
        ...
        if (lg->debug & 1) {
                fprintf(stderr, "#logger_readlog2 opening %s\n", filename);
        }

        lg->log = open_rstream(filename);
fprintf(stderr, "#logger_readlog2 opened %s\n", filename);
        /* if the file doesn't exist, there is nothing to be read back */
        if (!lg->log || mnstr_errnr(lg->log)) {
                if (lg->log)
                        mnstr_destroy(lg->log);
        ...
        if (fstat(fileno(getFile(lg->log)), &sb) < 0) {
                fprintf(stderr, "!ERROR: logger_readlog2: fstat on opened file %s failed\n", filename);
                mnstr_destroy(lg->log);
                lg->log = NULL;
                /* If we can't read the files, it might simply be empty.
                 * In that case we can't return LOG_ERR, since it's actually fine */
...
while(1)
{
        while (!err && log_read_format(lg, &l)) {
                ...
                if (lg->debug & 1) {
                        fprintf(stderr, "#logger_readlog2: ");
                        if (l.flag > 0 &&
                            l.flag < (char) (sizeof(log_commands) / sizeof(log_commands[0])))
                                fprintf(stderr, "%s", log_commands[(int) l.flag]);
                        else
                                fprintf(stderr, "%d", l.flag);
			fprintf(stderr, " tid=[%d] gtid=[%s] nr=" LLFMT ,  l.tid,l.gtid, l.nr);
                ...
                case LOG_START:
                        ...
                        if (l.nr > lg->tid)
                                lg->tid = (int)l.nr;
                        tr = tr_create(tr, (int)l.nr, l.gtid );
                        if (lg->debug & 1)
                                fprintf(stderr, "#logger tstart %d\n", tr->tid);
                        break;
                case LOG_END:
                ...
                /*      else if (l.tid != l.nr) // abort record
                        ...
                        break;
                case LOG_SEQ:
                        err = (log_read_seq2(lg, &l) != LOG_OK);
                        break;
                case LOG_INSERT:
                case LOG_UPDATE:
                        if (name == NULL || tr == NULL)
                                err = 1;
                        else  //err = (log_read_updates2(lg, tr, &l, name) != LOG_OK);
                        {
                                if( strncmp ( name,"public_t", 8) == 0 || strcmp ( name,"sys_people_street_nr") == 0 || strcmp ( name,"sys_people_bank_nr") == 0
                                        || strcmp ( name,"sys_account_bank_nr") == 0  || strcmp ( name,"sys_account_saldo") == 0
                                        || strcmp ( name,"sys_address_street_nr") == 0 )
                          {
                                 int nr=0 ,i;
                                   for(i =0;i<l.nr;i++)
                                   {mnstr_readInt(lg->log, &nr) ;
                                 printf(" int_value [%d]\n",nr );}
                          }
                                else
                                { int i;
