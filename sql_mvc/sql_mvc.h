...
#define m_prepare 	3
#define m_plan 		4
#define m_skip		5 /* ignore the query for example gp may send both create stmt and create external stmt */
...
// debug options for SQL
#define DEBUG_FILE_PLAN 256
#define DEBUG_DUMP_PLAN 512
#define DEBUG_MAL_PLAN  1024
#define DEBUG_DUMP_TRAN 2048

// debug_extend for GP
// Debug_abort_after_distributed_prepared
#define DEBUG_ABORT_AFTER_DISTRIBUTED_PREPARED 1
// Debug_abort_after_segment_prepared
#define DEBUG_ABORT_AFTER_SEGMENT_PREPARED 2

#define CRASH_BEFORE_PREPARED 1
#define CRASH_AFTER_PREPARED  2
#define CRASH_BEFORE_COMMIT_PREPARED 3
#define CRASH_AFTER_COMMIT_PREPARED 4
#define HANG_BEFORE_PREPARED 5
#define HANG_AFTER_PREPARED  6
#define HANG_BEFORE_COMMIT_PREPARED 7
#define HANG_AFTER_COMMIT_PREPARED 8
...
typedef struct mvc {
	...
	int debug;
	int debug_extend;	/* GP extend debug flag*/
  int debug_crash_point;  /* for crash point test*/
  int debug_hang_point;  /* for crash point test*/
	...
	int result_id;
	int current_redo_log_off; /* 1,not write redo log;  0 write redo log;default 0 */
	res_table *results;

	bool randomdist;	/* insert single row values for random table */
} mvc;
...
extern int mvc_prepare(mvc *c, int chain, const char *name ,char *logName, const char *gid);
extern int mvc_commit_prepare(mvc *c, int chain, const char *name,char *logName, const char *gid);
extern int mvc_abort_prepare(mvc *c, int chain, const char *name);
extern int mvc_rollback(mvc *c, int chain, const char *name);
extern int mvc_release(mvc *c, const char *name);
extern int mvc_recovery_commit_prepare(mvc *c, int chain, const char *name,char *logDir, const char *gid);
extern int mvc_recovery_abort_prepare(mvc *c, int chain, const char *name,char *logDir, const char *gid);
extern int mvc_retry_commit_prepare(mvc *c, int chain, const char *name,char *logDir, const char *gid);
extern int mvc_retry_abort_prepare(mvc *m, int chain, const char *name);
extern int mvc_gp_max_distributed_xid( long *ret );
extern int mvc_pg_prepared_xact( char *ret ,char*db);
extern sql_table *mvc_rename_table( mvc *m, const char *sname, const char *tname, const char *tnewname );
...
extern sql_table * mvc_alter_table_type(mvc *m, sql_table *t, int type);
